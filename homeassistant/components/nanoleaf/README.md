...
- Individual panels are now exposed as separate light entities with the format `light.<serial>_panel_<id>` instead of `light.<device>_panel_<id>`
- Per-panel control is now optional and can be configured via integration options
- Panel entities now appear grouped under the main Nanoleaf device in the UI
